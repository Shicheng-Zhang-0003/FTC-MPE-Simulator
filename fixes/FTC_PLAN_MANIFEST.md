...
- [x] **Robot Object & Drivetrains:** Tank and Mecanum IK. (Note: Mecanum currently uses a chassis-force workaround due to sphere wheels).
...
- [ ] **090: Cylinder Headers** — Add `object_cylinder` to enum, `cylinder_half_length` to struct, declare init/inertia functions.
- [ ] **091: Cylinder Implementation** — Cylinder inertia tensor (X-axis axle), `physics_world_add_cylinder`, broadphase bounding radius.
- [ ] **092: Cylinder–Floor Narrowphase** — Closest-point collision detection between cylinder rolling surface and ground plane.
- [ ] **093: Cylinder–Object Narrowphase** — Cylinder vs. Sphere and Cylinder vs. OBB collisions.
- [ ] **094: Anisotropic Friction Model** — Split tangential velocity into rolling (high grip) and axle (low slip) components. Add `roller_angle` for mecanum 45° rollers.
- [ ] **095: Real Drivetrain Rebuild** — Rewrite `robot.c` to use cylinder wheels. **Delete** `wheel_traction.c` raycast hack and `drivetrain_mecanum` chassis-force cheat.
- [ ] **096: Drivetrain Validation** — Tank drives straight, mecanum strafes via wheel contact, turning works, push-test resists lateral force.
...
- [ ] **100: Iterative Constraint Solve** — Move `revolute_solve` into the main solver iteration loop with accumulated impulses and warm-starting.
