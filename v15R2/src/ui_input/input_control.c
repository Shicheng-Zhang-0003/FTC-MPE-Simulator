...
gboolean on_keypress(GtkWidget *widget, GdkEventKey *event, gpointer user_data_stored) {
    ...
    if (event->keyval == GDK_KEY_w) {
        input_state->w_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_a) {
        input_state->a_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_s) {
        input_state->s_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_d) {
        input_state->d_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_e) {
        input_state->e_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_f) {
        input_state->f_key_pressed = true;
    }
    /* MPE_TASK_21_KEYBOARD_ONLY_KEYPRESS_BEGIN */
    if ((event->keyval == GDK_KEY_r) || (event->keyval == GDK_KEY_R)) {
        input_state->r_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_Delete) {
        input_state->delete_key_pressed = true;
    }
    if ((event->keyval == GDK_KEY_m) || (event->keyval == GDK_KEY_M)) {
        input_state->m_key_pressed = true;
    }
    ...
    if (event->keyval == GDK_KEY_F5) {
        input_state->stability_test_pressed = true;
    }
    if (event->keyval == GDK_KEY_F6) {
        input_state->sleep_wake_test_pressed = true;
    }
    if (event->keyval == GDK_KEY_F7) {
        input_state->editor_torture_pressed = true;
    }
    if (event->keyval == GDK_KEY_F8) {
        input_state->spawn_stress_pressed = true;
    }
    if (event->keyval == GDK_KEY_F9) {
        input_state->validation_report_pressed = true;
    }
    /* MPE_TASK_13_LONG_RUN_KEY_BEGIN */
    if (event->keyval == GDK_KEY_F10) {
        input_state->long_run_validation_pressed = true;
    }
    /* MPE_TASK_13_LONG_RUN_KEY_END */
    /* MPE_TASK_39_CONFIG_TORTURE_KEY_BEGIN */
    if (event->keyval == GDK_KEY_F11) {
        input_state->config_torture_pressed = true;
    }
    /* MPE_TASK_39_CONFIG_TORTURE_KEY_END */
    if (event->keyval == GDK_KEY_i) {
        input_state->i_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_j) {
        input_state->j_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_k) {
        input_state->k_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_l) {
        input_state->l_key_pressed = true;
    }
    if ((event->keyval == GDK_KEY_9) && (!config_menu_is_open())) {
        input_state->spawner_menu_level = 0;
        input_state->velocity_menu_level = 0;
        input_state->object_menu_level = 0;
        input_state->is_menu_open = !(input_state->is_menu_open);
    }
    if ((event->keyval == GDK_KEY_8) && (!config_menu_is_open())) {
        input_state->is_menu_open = false;
        input_state->velocity_menu_level = 0;
        input_state->object_menu_level = 0;
        if (input_state->spawner_menu_level > 0) {
            input_state->spawner_menu_level = 0;
        } else {
            input_state->spawner_menu_level = 1;
        }
    }
    if ((event->keyval == GDK_KEY_7) && (!config_menu_is_open())) {
        ...
        input_state->spawner_menu_level = 0;
        input_state->object_menu_level = 0;
        if (input_state->velocity_menu_level > 0) {
            input_state->velocity_menu_level = 0;
        } else {
            input_state->velocity_menu_level = 1;
        }
    }
    /* MPE_TASK_35_CONFIG_MENU_KEY_BEGIN */
    if ((event->keyval == GDK_KEY_6) && (!input_state->is_menu_open) && (input_state->object_menu_level == 0) &&
        (input_state->spawner_menu_level == 0) && (input_state->velocity_menu_level == 0)) {
        input_state->spawner_menu_level = 0;
        input_state->velocity_menu_level = 0;
        input_state->object_menu_level = 0;
    ...
    if ((event->keyval == GDK_KEY_1) && (!config_menu_is_open()) && (input_state->is_debug_mode_active) &&
        (!input_state->is_menu_open) && (input_state->spawner_menu_level == 0) &&
        (input_state->velocity_menu_level == 0) && (input_state->object_menu_level == 0)) {
        input_state->debug_terminal_pressed = true;
    }
    /* MPE_TASK_18_TERMINAL_KEY_END */
    if (input_state->is_menu_open) {
        if (event->keyval == GDK_KEY_1) {
            input_state->menu_1_pressed = true;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->menu_2_pressed = true;
        }
        if (event->keyval == GDK_KEY_3) {
            input_state->menu_3_pressed = true;
        }
        if (event->keyval == GDK_KEY_4) {
            input_state->menu_4_pressed = true;
        }
        if (event->keyval == GDK_KEY_5) {
            input_state->menu_5_pressed = true;
        }
        if (event->keyval == GDK_KEY_6) {
            input_state->menu_6_pressed = true;
        } /* MPE_TASK_35 */
    } // Menu Navigation
    ...
    if ((input_state->spawner_menu_level > 0) || (input_state->velocity_menu_level > 0) ||
        (input_state->object_menu_level > 0)) {
        if (event->keyval == GDK_KEY_Up) {
            input_state->up_arrow_pressed = true;
        }
        if (event->keyval == GDK_KEY_Down) {
            input_state->down_arrow_pressed = true;
        }
        if (event->keyval == GDK_KEY_Left) {
            input_state->left_arrow_pressed = true;
        }
        if (event->keyval == GDK_KEY_Right) {
            input_state->right_arrow_pressed = true;
        }
        if ((event->keyval == GDK_KEY_Return) || (event->keyval == GDK_KEY_KP_Enter)) {
            input_state->enter_key_pressed = true;
        }
    } // Spawner Menu Logic
    if (input_state->spawner_menu_level == 1) {
        if (event->keyval == GDK_KEY_1) {
            input_state->spawner_menu_level = 2;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->spawner_menu_level = 5;
        }
        if (event->keyval == GDK_KEY_3) {
            input_state->spawner_menu_level = 8;
        }
    } else if (input_state->spawner_menu_level == 2) {
        if (event->keyval == GDK_KEY_1) {
            input_state->spawner_menu_level = 3;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->spawner_menu_level = 4;
        }
    } else if (input_state->spawner_menu_level == 5) {
        if (event->keyval == GDK_KEY_1) {
            input_state->spawner_menu_level = 6;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->spawner_menu_level = 7;
        }
    } // Velocity Menu Logic
    if (input_state->velocity_menu_level == 1) {
        if (event->keyval == GDK_KEY_1) {
            input_state->velocity_menu_level = 2;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->velocity_menu_level = 10;
        }
        if (event->keyval == GDK_KEY_3) {
            input_state->velocity_menu_level = 20;
        }
    } else if (input_state->velocity_menu_level == 2) {
        if (event->keyval == GDK_KEY_1) {
            input_state->velocity_menu_level = 3;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->velocity_menu_level = 4;
        }
    } else if (input_state->velocity_menu_level == 20) {
        if (event->keyval == GDK_KEY_1) {
            input_state->velocity_menu_level = 21;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->velocity_menu_level = 22;
        }
        if (event->keyval == GDK_KEY_3) {
            input_state->velocity_menu_level = 23;
        }
    } else if (input_state->velocity_menu_level == 10) {
        if (event->keyval == GDK_KEY_1) {
            input_state->velocity_menu_level = 11;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->velocity_menu_level = 12;
        }
    } // Selected Object Menu Logic
    if (input_state->object_menu_level == 1) {
        if (event->keyval == GDK_KEY_1) {
            input_state->object_menu_level = 2;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->object_menu_level = 3;
        }
        if (event->keyval == GDK_KEY_3) {
            input_state->object_menu_level = 4;
        }
        if (event->keyval == GDK_KEY_4) {
            input_state->object_menu_level = 5;
        }
        if (event->keyval == GDK_KEY_5) {
            input_state->object_menu_level = 6;
        }
        if (event->keyval == GDK_KEY_6) {
            if (input_state->marked_joint_object_index != -1 &&
                input_state->marked_joint_object_index != selected_object) {
        ...
        if (event->keyval == GDK_KEY_1) {
            input_state->object_menu_level = 81;
        }
        if (event->keyval == GDK_KEY_2) {
            input_state->object_menu_level = 82;
        }
        if (event->keyval == GDK_KEY_3) {
            input_state->object_menu_level = 83;
        }
        if (event->keyval == GDK_KEY_4) {
            input_state->object_menu_level = 84;
        }
        if (event->keyval == GDK_KEY_5) {
            input_state->object_menu_level = 85;
        }
        if (event->keyval == GDK_KEY_6) {
            input_state->object_menu_level = 86;
        }
        if ((event->keyval == GDK_KEY_7) && (!config_menu_is_open())) {
            input_state->object_menu_level = 87;
        }
        if ((event->keyval == GDK_KEY_8) && (!config_menu_is_open())) {
            input_state->object_menu_level = 88;
        }
    } /* MPE_TASK_22_ENTER_SPAWN_KEYPRESS_BEGIN */
    if (((event->keyval == GDK_KEY_Return) || (event->keyval == GDK_KEY_KP_Enter)) && (!input_state->is_menu_open) &&
        (input_state->spawner_menu_level == 0) && (input_state->velocity_menu_level == 0) &&
        (input_state->object_menu_level == 0)) {
    ...
    if (event->keyval == GDK_KEY_space) {
        input_state->space_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_Shift_L) {
        input_state->shift_key_pressed = true;
    }
    if (event->keyval == GDK_KEY_Escape) {
        input_state->escape_key_pressed = true;
    }
    if ((event->keyval == GDK_KEY_0) && (!config_menu_is_open())) {
...
gboolean on_mouse_movements(GtkWidget *widget, GdkEventMotion *event, gpointer user_data_stored) {
    ...
    if (input_state->is_mouse_locked) {
        ...
        if ((current_mouse_x == last_warp_x) && (current_mouse_y == last_warp_y)) {
            return FALSE;
        }
...
gboolean on_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data_stored) {
    ...
    if (event->button == 2) {
        input_state->middle_mouse_button_clicked = true;
    }
    if (event->button == 3) {
        input_state->right_mouse_button_clicked = true;
    }
    if (!(input_state->is_mouse_locked)) {
        input_state->mouse_delta_x = 0.0f;
        input_state->mouse_delta_y = 0.0f;
